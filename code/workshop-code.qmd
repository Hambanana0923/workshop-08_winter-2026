...
[deployed html url](https://hambanana0923.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
drought_exp_clean <- drought_exp |> 
  # store drought_exp dataframe in new object drought_exp_clean
  clean_names() |> 
  # make column names all lowercase and underscores 
  ...
  # adding new column for treatment and change names of species into fully typed out name from abbreviation
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  # Making sure that thsese categorical variables are factors and setting factor level order
...
ggplot(
  ...
  aes(x = sla,
    ...
    labs(
      ...
      y = "Total Mass (g)",
      title = "as SLA increase, total mass increases"
    ) 
```
...
```{r}
#| label: plant mass v water treatments
...
ggplot(
  ...
  labs(
    x = "Water Treatment",
    y = "Total Mass (g)",
    title = "Mean Well Watered mass is greater than Mean Drought Stressed Mass"
  ) +
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Species",
       y = "Total Mass (g)",
       title = "Mean Plant Mass Differs between Species") +
  theme(legend.position = "none") +
  scale_color_manual(values = c(
    "Lotus scorparius" = lotsco_col,
    "Eschscholzia californica" = esccal_col, # California poppy
    "Penstemon centranthifolius" = pencen_col,# Scarlet bugler
    "Grindelia camporum" = gricam_col,# Gumweed
    "Salvia leucophylla" = salleu_col, # purple sage
    "Stipa pulchra" = stipul_col # purple needlegrass
  )) + 
